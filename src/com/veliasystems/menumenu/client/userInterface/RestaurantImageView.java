...
public class RestaurantImageView extends MyPage{
	...
	protected void onPageShow() {
		...
		if(!loaded){
			SwipeView swipeView = new SwipeView(restaurant, ImageType.PROFILE, this);
		...
		backButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				PagesController.showWaitPanel();
				JQMContext.changePage(new CityInfoScreen(restaurant.getCity()), Transition.SLIDE);
			}
		});
	...
	if(container == null){
		...
		adminPanelWrapper.getElement().getStyle().setDisplay(Display.INLINE_BLOCK);
